...
class Config:
    def __init__(self):
        self._SOURCES_PATH = self._SOURCES_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sources.yml")
    ...
    def get_source(self, name: str):
        if name not in self.sources:
            raise KeyError(f"Source '{name}' not found in sources.yml. Available: {list(self.sources.keys())}")
